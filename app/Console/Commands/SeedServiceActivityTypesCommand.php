...
use App\Models\ServiceActivityTypeTranslation;
use Illuminate\Console\Command;

/**
 * Seeds cat_service_activity_types and their Spanish translations from a fixed list.
 * Section headers are the "code" of the service_activity_type_category (e.g. general, culture).
 * Categories must already exist with matching codes. Uses only the languages table.
 *
 * Run from project root (Debian): php artisan service-activity-types:seed --language-id=2
 * Idempotent: skips types that already exist (by code).
 */
class SeedServiceActivityTypesCommand extends Command
{
    protected $signature = 'service-activity-types:seed
                            {--language-id= : Id of the Spanish language in the languages table (required)}
                            {--dry-run : Show what would be created without writing}';

    protected $description = 'Seed catalogue activity types and Spanish translations from the predefined list';
    ...
    public function handle(): int
    {
        ...
        foreach ($this->data as $categoryCode => $types) {
            $category = ServiceActivityTypeCategory::query()
            ...
            foreach ($types as $index => $item) {
                ...
                $exists = ServiceActivityType::query()->where('code', $code)->exists();
                if ($exists) {
                    $skipped++;
                ...
                if (! $dryRun) {
                    $type = ServiceActivityType::create([
                        'code' => $code,
                        'service_activity_type_category_id' => $category->id,
                        'sort_order' => $index + 1,
                        'active' => true,
                    ]);

                    ServiceActivityTypeTranslation::create([
        ...
        $this->info($dryRun
            ? "Would create {$created} activity types (skip {$skipped} existing)."
            : "Created {$created} activity types, skipped {$skipped} existing.");
