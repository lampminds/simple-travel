...
use App\Models\ServiceExperienceTranslation;
use Illuminate\Console\Command;

/**
 * Seeds cat_service_experiences and their Spanish translations from a fixed list.
 ...
 *
 * Run from project root (Debian): php artisan service-experiences:seed --language-id=2
 * Idempotent: skips experiences that already exist (by code).
 */
class SeedServiceExperiencesCommand extends Command
{
    protected $signature = 'service-experiences:seed
                            {--language-id= : Id of the Spanish language in the languages table (required)}
                            {--dry-run : Show what would be created without writing}';

    protected $description = 'Seed service experiences and Spanish translations from the predefined list';
    ...
    public function handle(): int
    {
        ...
        foreach ($this->data as $categoryNameEs => $activities) {
            $category = ServiceExperienceCategory::query()
            ...
            foreach ($activities as $index => $item) {
                ...
                $exists = ServiceExperience::query()->where('code', $code)->exists();
                if ($exists) {
                    $skipped++;
                ...
                if (! $dryRun) {
                    $experience = ServiceExperience::create([
                        'code' => $code,
                        'service_experience_category_id' => $category->id,
                        'sort_order' => $index + 1,
                        'active' => true,
                    ]);

                    ServiceExperienceTranslation::create([
                        'service_experience_id' => $experience->id,
        ...
        $this->info($dryRun
            ? "Would create {$created} experiences (skip {$skipped} existing)."
            : "Created {$created} experiences, skipped {$skipped} existing.");
