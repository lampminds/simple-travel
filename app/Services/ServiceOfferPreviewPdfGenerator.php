...
/**
 * Renders the operator service-offer preview sheet as a downloadable PDF.
 */
final class ServiceOfferPreviewPdfGenerator
{
    public function __construct(
        private readonly ServiceOfferPreviewBuilder $previewBuilder,
        private readonly OperatorVariantPriceResolver $priceResolver,
    ) {
    }

    public function downloadResponse(ServiceOffer $offer, bool $includePhotos = true): Response
    {
        $offer->loadMissing('serviceVariant');
        abort_unless($offer->serviceVariant !== null, 404);
        ...
        $preview = $this->previewBuilder->build($offer, $operatorPrice);
        ...
        $html = view('account.service-offers.operator.preview-pdf', [
            'preview' => $preview,
        ])->render();
        ...
        $filename = Str::slug(Str::limit((string) ($preview['title'] ?? 'service'), 80, ''));
        if ($filename === '') {
            $filename = 'service-offer-'.($offer->uuid ?: $offer->id);
        }
    ...
    private function stripPreviewMedia(array $preview): array
    {
        ...
        if (! isset($preview['sections']) || ! is_array($preview['sections'])) {
            return $preview;
        }

        $preview['sections'] = array_values(array_filter(
            $preview['sections'],
            static fn (mixed $section): bool => is_array($section)
                && (($section['key'] ?? '') !== ServiceCopySections::IMAGES),
        ));
    ...
    private function applyFooter(Dompdf $dompdf, array $footerMeta): void
    {
        ...
        $canvas->page_text(
            $width - 130,
            $y,
            __('account.service_offers.operator_preview_pdf_pages'),
            $font,
            $size,
            $color,
        );
    ...
    private function embedPreviewMedia(array $preview): array
    {
        ...
        if (isset($preview['sections']) && is_array($preview['sections'])) {
            foreach ($preview['sections'] as $sectionIndex => $section) {
                if (! is_array($section) || ! isset($section['galleries']) || ! is_array($section['galleries'])) {
                    continue;
                }
                ...
                foreach ($section['galleries'] as $galleryIndex => $gallery) {
                    ...
                    foreach ($gallery['images'] as $imageIndex => $image) {
                        if (is_array($image) && isset($image['url'])) {
                            $preview['sections'][$sectionIndex]['galleries'][$galleryIndex]['images'][$imageIndex] = array_merge(
                                $image,
