@php
    ...
    $isEdit = $rule !== null;
    $catalogLabel = $catalog->displayLabel();
    ...
    if ($timeSlots === [] && $catalog->usesTimeSlotInventory()) {
        $timeSlots = [['start_time' => '', 'end_time' => '', 'capacity' => '', 'cutoff_minutes' => '', 'active' => true]];
    }
@endphp

@extends('layouts.base', ['title' => $isEdit ? __('account.package_availability.rule_edit_page_title') : __('account.package_availability.rule_create_page_title')])
            ...
            <div class="row">
                <div class="col-lg-12">
                    <x-account-page-header
                        :title="$isEdit ? __('account.package_availability.rule_edit_heading') : __('account.package_availability.rule_create_heading')"
                        :subtitle="$catalogLabel"
                        :instructions="__('account.package_availability.rule_form_instructions')"
                    />
                </div>
            </div>
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ $submitRoute }}">
                                        ...
                                        <div class="col-12">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center mb-2">
                                                ...
                                                <button type="button" class="btn btn-sm btn-outline-secondary" id="add-time-slot">
                                                    {{ __('account.package_availability.add_time_slot') }}
                                                </button>
                                            </div>
                                            <div id="time-slots-wrap">
                                                @foreach ($timeSlots as $index => $slot)
                                                    ...
                                                    <div class="row g-2 align-items-end mb-2 time-slot-row" data-index="{{ $index }}">
                                                        ...
                                                        <div class="col-md-2">
                                                            <button type="button" class="btn btn-sm btn-outline-danger remove-time-slot">{{ __('account.package_availability.remove_time_slot') }}</button>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
                                ...
                                <div class="d-flex flex-wrap gap-2 mt-4">
                                    <a class="btn btn-light" href="{{ $cancelRoute }}">{{ __('account.package_availability.cancel_button') }}</a>
                                    <button type="submit" class="btn btn-primary">
                                        {{ $isEdit ? __('account.package_availability.update_button') : __('account.package_availability.save_button') }}
                                    </button>
                                </div>
                            </form>
                        </div>
