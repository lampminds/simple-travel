...
@section('content')
                    ...
                    <div class="page-title">
                        ...
                        @if ($isEdit)
                            <p class="mt-2 mb-0 small">
                                <a href="{{ route('account.price-lists.assignments.edit', $priceList) }}">{{ __('account.price_lists.assignments_link_from_edit') }}</a>
                            </p>
                        @endif
                    </div>
                </div>
                                    ...
                                    >
                                        ...
                                        <div class="table-responsive">
                                            <table class="table align-middle" id="items-table">
                                                <thead>
                                                    <tr>
                                                        <th>{{ __('account.price_lists.fields.variant') }}</th>
                                                        <th>{{ __('account.price_lists.fields.pricing_mode') }}</th>
                                                        <th>{{ __('account.price_lists.fields.application_mode') }}</th>
                                                ...
                                                <tbody id="items-body">
                                                    @foreach($items as $index => $item)
                                                        <tr>
                                                            <td>
                                                                <select name="items[{{ $index }}][service_variant_id]" class="form-select" required>
                                                            ...
                                                            <td>
                                                                <select name="items[{{ $index }}][application_mode]" class="form-select" required>
                                                                    <option value="compose" @selected(($item['application_mode'] ?? '') === 'compose')>{{ __('account.price_lists.application_mode.compose') }}</option>
                                                                    <option value="final" @selected(($item['application_mode'] ?? '') === 'final')>{{ __('account.price_lists.application_mode.final') }}</option>
                                                                </select>
                                                            </td>
                                                            <td>
                                                                <input
    ...
    <script>
        (function () {
            ...
            const hintAmountCompose = @json(__('account.price_lists.hints.amount_compose'));
            const hintAmountFinal = @json(__('account.price_lists.hints.amount_final'));
            const hintPercentageCompose = @json(__('account.price_lists.hints.percentage_compose'));
            ...
            function applyPricingBehavior(row) {
                ...
                const applicationModeSelect = row.querySelector('select[name$="[application_mode]"]');
                const priceInput = row.querySelector('[data-role="item-price"]');
                const priceHelp = row.querySelector('[data-role="item-price-help"]');

                if (!pricingModeSelect || !applicationModeSelect || !priceInput || !priceHelp) {
                    return;
                }

                const isPercentage = pricingModeSelect.value === modePercentage;
                const finalOption = applicationModeSelect.querySelector('option[value="final"]');

                if (finalOption) {
                    finalOption.disabled = isPercentage;
                }

                if (isPercentage && applicationModeSelect.value !== modeCompose) {
                    applicationModeSelect.value = modeCompose;
                }

                priceInput.placeholder = isPercentage ? percentagePlaceholder : amountPlaceholder;

                if (isPercentage) {
                    priceHelp.textContent = hintPercentageCompose;
                    return;
                }

                priceHelp.textContent = applicationModeSelect.value === modeCompose
                    ? hintAmountCompose
                    : hintAmountFinal;
            }
            ...
            function bindRowEvents(row) {
                ...
                if (applicationModeSelect) {
                    applicationModeSelect.addEventListener('change', () => applyPricingBehavior(row));
                }
            ...
            function addRow() {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <select name="items[${index}][application_mode]" class="form-select" required>
                            <option value="compose">{{ __('account.price_lists.application_mode.compose') }}</option>
                            <option value="final">{{ __('account.price_lists.application_mode.final') }}</option>
                        </select>
                    </td>
                    <td>
