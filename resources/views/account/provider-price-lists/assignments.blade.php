@php
    ...
    $hasOperators = count($operatorOptions) > 0;
    ...
    if (is_array($assignmentsOld)) {
        ...
        $assignments = $priceList->assignments
            ->map(fn ($a) => [
                'operator_account_id' => $a->operator_id,
...
@extends('layouts.base', ['title' => __('account.price_lists.assignments_page_title', ['name' => $priceList->name])])
            ...
            <div class="row">
                <div class="col-lg-12">
                    <x-account-page-header
                        :title="__('account.price_lists.assignments_heading')"
                        :subtitle="__('account.price_lists.assignments_subtitle', ['name' => $priceList->name])"
                        :instructions="[
                            __('account.price_lists.assignments_instructions'),
                            __('account.price_lists.assignments_help'),
                        ]"
                    />
                </div>
            </div>
            ...
            <div class="row mt-3">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            @if ($hasOperators)
                                <form method="POST" action="{{ route('account.provider-price-lists.assignments.update', $priceList) }}">
                                    @csrf
                                    @method('PUT')

                                    <p class="small text-muted mb-3">
                                        {{ __('account.price_lists.fields.adjustment_value') }}: 12{{ $thousandsSeparator }}345{{ $decimalSeparator }}{{ str_repeat('0', max(1, $priceDecimals)) }}
                                    </p>

                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                        <h5 class="mb-0">{{ __('account.price_lists.assignments_rows_title') }}</h5>
                                        <button type="button" class="btn btn-sm btn-outline-primary" id="btn-add-assignment">
                                            {{ __('account.price_lists.add_assignment_button') }}
                                        </button>
                                    </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table align-middle" id="assignments-table">
                                            <thead>
                                                <tr>
                                                    <th>{{ __('account.price_lists.fields.operator') }}</th>
                                                    <th>{{ __('account.price_lists.fields.adjustment_type') }}</th>
                                                    <th>{{ __('account.price_lists.fields.adjustment_value') }}</th>
                                                    <th>{{ __('account.price_lists.fields.valid_from') }}</th>
                                                    <th>{{ __('account.price_lists.fields.valid_to') }}</th>
                                                    <th>{{ __('account.price_lists.fields.assignment_active') }}</th>
                                            ...
                                            <tbody id="assignments-body">
                                                @foreach ($assignments as $aIndex => $arow)
                                                    <tr>
                                                        <td>
                                                            <select name="assignments[{{ $aIndex }}][operator_account_id]" class="form-select">
                                                                <option value="">{{ __('account.price_lists.fields.operator_placeholder') }}</option>
                                                                @foreach ($operatorOptions as $opId => $opLabel)
                                                                    <option value="{{ $opId }}" @selected((string) ($arow['operator_account_id'] ?? '') === (string) $opId)>
                                                                        {{ $opLabel }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                            @error('assignments.'.$aIndex.'.operator_account_id')
                                                                <div class="text-danger small">{{ $message }}</div>
                                                            @enderror
                                                        </td>
                                                        <td>
                                                            <select name="assignments[{{ $aIndex }}][adjustment_type]" class="form-select">
                                                                <option value="none" @selected(($arow['adjustment_type'] ?? 'none') === 'none')>{{ __('account.price_lists.assignment_adjustment.none') }}</option>
                                                                <option value="percentage" @selected(($arow['adjustment_type'] ?? '') === 'percentage')>{{ __('account.price_lists.assignment_adjustment.percentage') }}</option>
                                                                <option value="fixed" @selected(($arow['adjustment_type'] ?? '') === 'fixed')>{{ __('account.price_lists.assignment_adjustment.fixed') }}</option>
                                                            </select>
                                                        ...
                                                        <td>
                                                            <input
                                                                ...
                                                                placeholder="{{ __('account.price_lists.fields.adjustment_value_placeholder') }}"
                                                            >
                                                        ...
                                                        <td class="text-end">
                                                            <button type="button" class="btn btn-sm btn-outline-danger btn-remove-assignment">
                                                                {{ __('account.price_lists.remove_assignment_button') }}
                                                            </button>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                    ...
                                    <div class="d-flex flex-wrap gap-2 mt-4">
                                        <a class="btn btn-light" href="{{ route('account.provider-price-lists.index') }}">{{ __('account.price_lists.assignments_cancel') }}</a>
                                        <a class="btn btn-outline-secondary" href="{{ route('account.provider-price-lists.edit', $priceList) }}">{{ __('account.price_lists.assignments_edit_prices') }}</a>
                                        <button type="submit" class="btn btn-primary">{{ __('account.price_lists.assignments_save') }}</button>
                                    </div>
                                </form>
                            @else
                                <p class="text-muted mb-3">{{ __('account.price_lists.assignments_no_operators') }}</p>
    ...
    @if ($hasOperators)
        <script>
            (function () {
                ...
                const operatorOptionsHtml = @json(collect($operatorOptions)->map(
                    fn ($label, $id) => '<option value="'.e((string) $id).'">'.e($label).'</option>'
                )->implode(''));

                const operatorPlaceholder = @json(__('account.price_lists.fields.operator_placeholder'));
                const adjustmentNone = @json(__('account.price_lists.assignment_adjustment.none'));
                const adjustmentPercentage = @json(__('account.price_lists.assignment_adjustment.percentage'));
                const adjustmentFixed = @json(__('account.price_lists.assignment_adjustment.fixed'));
                const adjustmentValuePlaceholder = @json(__('account.price_lists.fields.adjustment_value_placeholder'));
                const removeAssignmentLabel = @json(__('account.price_lists.remove_assignment_button'));
                ...
                function addAssignmentRow() {
                    ...
                    row.innerHTML = `
                        <td>
                            <select name="assignments[${index}][operator_account_id]" class="form-select">
                                <option value="">${operatorPlaceholder}</option>
                                ${operatorOptionsHtml}
                            </select>
                        </td>
                        <td>
